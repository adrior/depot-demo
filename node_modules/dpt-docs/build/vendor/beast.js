...
;(function () {
    ...
    Beast.parseBML = function (text) {
        ...
        do {
            ...
            if (matched[matched.length - 2] === '/') {
                ...
                do {
                    bmlEndsAt = indexOffset === 0 ? textPortion.search(closedNode) : textAfterClosedNode.search(closedNode) + indexOffset;
    ...
    function ParseBMLNode(text, single, isRoot) {
        text = text.substr(1);
        text = text.substr(0, text.length - (single ? 2 : 1));
    ...
    BemNode.prototype = {
         ...
         * @value  boolean
         */
        noElems: function noElems(value) {
            this._noElems = value;
        ...
        parentBlock: function parentBlock(bemNode, dontAffectChildren) {
            if (bemNode) {
                if (this._isElem && bemNode instanceof BemNode && bemNode !== this._parentBlock && !this._forcedParentBlock) {
                ...
                return this._implementedNode ? this._implementedNode._parentBlock : this._parentBlock;
            }
        },
         ...
         * @defaults     object
         */
        _extendProperty: function _extendProperty(propertyName, defaults) {
         ...
         * @handler  function
         */
        onWin: function onWin(events, handler) {
            var eventsArray = events.split(' ');
            for (var i = 0, ii = eventsArray.length; i < ii; i++) {
                (function (bemNode, event) {
         ...
         * @handler  function
         */
        onMod: function onMod(modName, modValue, handler) {
        ...
        trigger: function trigger(eventName, data) {
            if (this._domNode) {
                this._domNode.dispatchEvent(data ? new CustomEvent(eventName, { detail: data }) : new Event(eventName));
            }
        ...
        triggerWin: function triggerWin(eventName, data) {
            if (this._domNode) {
                ...
                window.dispatchEvent(data ? new CustomEvent(eventName, { detail: data }) : new Event(eventName));
            }
         ...
         */
        remove: function remove(dontUnlink) {
         ...
         */
        replaceWith: function replaceWith(bemNode) {
         ...
         */
        implementWith: function implementWith(bemNode) {
            this._setDomNodeClasses();
        ...
        elem: function elem() {
            ...
            for (var i = 0, ii = this._elems.length, elem; i < ii; i++) {
                ...
                for (var j = 0, jj = arguments.length; j < jj; j++) {
                    if (elem._nodeName === arguments[j] || elem._implementedNode && elem._implementedNode._nodeName === arguments[j]) {
                        elems.push(elem);
                    }
                }
            }
         ...
         */
        getWithContext: function getWithContext() {
        ...
        clone: function clone() {
            ...
            for (var key in this) {
                if (key === '_children') {
                    var cloneChildren = [];
                    for (var i = 0, ii = this._children.length; i < ii; i++) {
                        cloneChildren.push(this._children[i] instanceof BemNode ? this._children[i].clone() : this._children[i]);
                    }
        ...
        _renderChildWithIndex: function _renderChildWithIndex(index) {
            ...
            if (child instanceof BemNode) {
                ...
                this._domNode.appendChild(document.createTextNode(child));
            }
        },
         ...
         */
        _callModHandlers: function _callModHandlers(modName, modValue, data, context) {
            var handlers;
         ...
         * Sets DOM classes
         */
        _setDomNodeClasses: function _setDomNodeClasses(returnClassNameOnly) {
         ...
         * Sets DOM CSS
         */
        _setDomNodeCSS: function _setDomNodeCSS() {
            var css = '';
        ...
        toStringOfFunctions: function toStringOfFunctions() {
            ...
            return 'Beast.node(' + '"' + this._nodeName + '",' + (attr === '{}' ? 'null' : attr) + (children ? ',' + children + ')' : ')');
        },
